...
export default function TaskPosterInfo({
    ...
    const fetchPosterProfile = async () => {
      ...
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('profiles').select('*').eq('id', userId) // Changed from taskerId
        .single();
        if (error) {
  ...
  return <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
      ...
      <div className="flex items-center space-x-4 mb-4">
        <Avatar>
          {profile.avatar_url ? <AvatarImage src={profile.avatar_url} alt={profile.full_name || "Task Poster"} /> : <AvatarFallback>{profile.full_name?.charAt(0) || "TP"}</AvatarFallback>}
        </Avatar>
        <div>
          <p className="text-lg font-medium">{profile.full_name || "Task Poster"}</p>
          {profile.location && <div className="flex items-center text-gray-500">
      ...
      <div className="space-y-3">
        ...
        {profile.created_at && <div className="flex items-center text-gray-500">
